...
| Application FolderName | Build tool | sdk minimun |
| :-: | :-: | :-: |
| ActivityLifecycle | Ant | 11 |
| AndroidAcceleration | Ant | 11 |
| AndroidReceiveSMS | Ant | 11 |
| CamTestActivity | Ant | 11 |
| ConectivityListener | Ant | 11 |
| PlayAudio | Ant | 11 |
| PlayVideo | Ant | 11 |
| SendingToAnotherApp | Ant | 11 |
| SendSMS | Ant | 11 |
| ButtonList | Gradle |  15 |
| Swipe | Gradle | 15 |
| Webview | Gradle | 15 |
...
To know if the tools has run the apps correctly it can be found on the file `06_tool.logcat` of each run `/androtest-on-docker/results/<machine_name>/<tool_name>/<arch>/<api>/<app_folderName>/run<i>`, searching for the next texts:

| App FolderName | Search for |
|:-:|:-:|
| ActivityLifecycle | Method Covered: PAUSE `or` Method Covered: RESUME `or` Method Covered: STOP `or` Method Covered: START |
| AndroidAcceleration | Acceleration change succesfully, Yahoo! |
| AndroidReceiveSMS | Text Message Received |
| CamTestActivity | Took the picture Yay! |
| ConectivityListener | change in conectivity found Test Succesful |
| PlayAudio | Video Reproduced |
| PlayVideo |  |
| SendingToAnotherApp | Intent is safe, Sending user to maps `or` Failed to create a safe intent, Did not send to Maps `or` Adress is Empty Intent can not be created|
| SendSMS | SMS Sent! `or` SMS failed, please try agein later! |
| ButtonList| button number: <0 to 14>|
| Swipe| Central button `&` Right button `&` Last button|
| Webview | |